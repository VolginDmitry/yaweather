...
try:
    ...
    night_data = pd.DataFrame([(json_data['forecasts'][f]['date'], 'night',
                                json_data['forecasts'][f]['parts']['night']['temp_min'],
                                json_data['forecasts'][f]['parts']['night']['temp_avg'],
                                json_data['forecasts'][f]['parts']['night']['temp_max'],
                                json_data['forecasts'][f]['parts']['night']['pressure_mm'],
                                json_data['forecasts'][f]['parts']['night']['humidity'],
                                json_data['forecasts'][f]['parts']['night']['condition'])
                               for f in range(len(json_data['forecasts']))],
    ...
    morning_data = pd.DataFrame([(json_data['forecasts'][f]['date'], 'morning',
                                  json_data['forecasts'][f]['parts']['morning']['temp_min'],
                                  json_data['forecasts'][f]['parts']['morning']['temp_avg'],
                                  json_data['forecasts'][f]['parts']['morning']['temp_max'],
                                  json_data['forecasts'][f]['parts']['morning']['pressure_mm'],
                                  json_data['forecasts'][f]['parts']['morning']['humidity'],
                                  json_data['forecasts'][f]['parts']['morning']['condition'])
                                 for f in range(len(json_data['forecasts']))],
                                columns=['date', 'time_of_day', 'temp_min', 'temp_avg', 'temp_max', 'pressure_mm',
                                         'humidity', 'condition'])

    # формируем датафрейм с необходимымыми признаками по дневным данным
    day_data = pd.DataFrame([(json_data['forecasts'][f]['date'], 'day',
                              json_data['forecasts'][f]['parts']['day']['temp_min'],
                              json_data['forecasts'][f]['parts']['day']['temp_avg'],
                              json_data['forecasts'][f]['parts']['day']['temp_max'],
                              json_data['forecasts'][f]['parts']['day']['pressure_mm'],
                              json_data['forecasts'][f]['parts']['day']['humidity'],
                              json_data['forecasts'][f]['parts']['day']['condition'])
                             for f in range(len(json_data['forecasts']))],
                            columns=['date', 'time_of_day', 'temp_min', 'temp_avg', 'temp_max', 'pressure_mm',
                                     'humidity', 'condition'])

    # формируем датафрейм с необходимымыми признаками по вечерним данным
    evening_data = pd.DataFrame([(json_data['forecasts'][f]['date'], 'evening',
                                  json_data['forecasts'][f]['parts']['evening']['temp_min'],
                                  json_data['forecasts'][f]['parts']['evening']['temp_avg'],
                                  json_data['forecasts'][f]['parts']['evening']['temp_max'],
                                  json_data['forecasts'][f]['parts']['evening']['pressure_mm'],
                                  json_data['forecasts'][f]['parts']['evening']['humidity'],
                                  json_data['forecasts'][f]['parts']['evening']['condition'])
                                 for f in range(len(json_data['forecasts']))],
                                columns=['date', 'time_of_day', 'temp_min', 'temp_avg', 'temp_max', 'pressure_mm',
                                         'humidity', 'condition'])

    # формируем датафрейм с информацией по магнитному полю
    magnetic_field_data = pd.DataFrame(
        [(json_data['forecasts'][f]['date'], json_data['forecasts'][f]['biomet']['index']) for f in range(4)],
    ...
    df_final.columns = ['Дата', 'Время суток', 'Минимальная температура', 'Максимальная температура',
                        'Cредня температуру за световой день', 'Описание погоды', 'Атмосферное давление_мм',
